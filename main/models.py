...
from .managers import ObjectManager,QuarterelyMonthlyBudgetObjectManager,YearlyBudgetObjectManager
...
class AayatNiryat(FPNBaseModel):
    ...
    broad_type=models.PositiveIntegerField(choices=commons.BROAD_TYPE_CHOICES,null=True)
    related_month=models.PositiveIntegerField(choices=commons.MONTHS_CHOICES,null=True)
    type = models.CharField(max_length=100, choices=commons.TYPES_CHOICES)
    ekai = models.CharField(max_length=10, choices=commons.EKAI_CHOICES, blank=True)
    ...
    def aayatniryat_total_sum(self):
        fields = [
            'm_pragati', 'pratikul_n', 'pratikul_p',
        ]
        total_sum = sum(getattr(self, field) for field in fields)
        return total_sum   
    ...
    def bulk_create(cls,**kwargs):
        return cls.objects.bulk_create(**kwargs)
...
#     budget_type=models.PositiveIntegerField(choices=commons.BUDGET_TYPES)#Budget or expenses or rajasho
#     time_type=models.PositiveIntegerField(choices=commons.TIME_TYPES) #whether monthly or quarterely or yearly
...
#For budget handling
class Budget(FPNBaseModel):
    pujigat=models.FloatField(default=0,blank=True)
    chalugat=models.FloatField(default=0,blank=True)
    aarthik_barsa=models.CharField(max_length=12,default='80/81')
    @property
    def total_budget(self):
        return self.pujigat+self.chalugat
    class Meta:
        abstract = True

class YearlyBudget(Budget):
    objects=YearlyBudgetObjectManager()
    pass

 

class QuarterelyBudget(Budget):
    month_or_quarter=models.PositiveIntegerField(choices=commons.QUARTER_CHOICES)
    objects=QuarterelyMonthlyBudgetObjectManager()
    @classmethod
    def create(cls,**kwargs):
        return cls.objects.create(**kwargs)

class MonthlyExpense(Budget):
    month_or_quarter=models.PositiveIntegerField(choices=commons.MONTHS_CHOICES)
    objects=QuarterelyMonthlyBudgetObjectManager()

class Rajaswo(FPNBaseModel):
    aarthik_barsa=models.CharField(max_length=20,default='80/81')
    rajaswo_collected=models.FloatField(default=0,blank=True)
    month_or_quarter=models.PositiveIntegerField(choices=commons.MONTHS_CHOICES)
    objects=QuarterelyMonthlyBudgetObjectManager()
...
class DetailHotel(FPNBaseModel):
    ...
    j_miti = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
    n_miti = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
...
class DetailAnugaman(FPNBaseModel):
    ...
    miti = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
    naam = models.CharField(max_length=255, default="", blank=True)
    thegana = models.CharField(max_length=255, default="", blank=True)
...
class DetailMudha(FPNBaseModel):
    ...
    p_miti = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
    b_miti = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
    ...
    m_miti = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
...
class DetailRbpa(FPNBaseModel):
    ...
    date = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
    related_month=models.PositiveIntegerField(choices=commons.MONTHS_CHOICES,default=0, blank=True)
    broad_type=models.PositiveIntegerField(choices=commons.RBPA_TYPES,default=0, blank=True)
    country1 = models.CharField(max_length=255, default="", blank=True)
    ...
    country2 = models.CharField(max_length=255, default="", blank=True)
    sample3 = models.PositiveIntegerField(default=0, blank=True)
    quantity3 = models.PositiveIntegerField(default=0, blank=True)
    country3 = models.CharField(max_length=255, default="", blank=True)

    objects=ObjectManager()
    #sample Count Sum
    def total_sample_sum(self):
        sample_fields = [
            'sample1', 'sample2', 'sample3',
        ]
        total_sample_sum = sum(getattr(self, sample_field) for sample_field in sample_fields)
        return total_sample_sum
    #quantity sum
    def total_quantity_sum(self):
        quantity_fields=[
            'quantity1','quantity2','quantity3',
        ]
        total_quantity_sum = sum(getattr(self, quantity_field) for quantity_field in quantity_fields)
...
class DetailGunaso(FPNBaseModel):
    ...
    s_miti = models.CharField(max_length=10, default="", blank=True, validators=[validate_nepali_date])
