from .models import (AayatNiryat, NamunaBibaran, AnugamanBibaran, Logobitaran,
                    ...
                    UdyogSifaris, UjuriGunaso, DetailRbpa, KhadyaPrasodhan)

from django.db.models import Sum
import nepali_datetime
from fpn.commons import quarter_dict
...
def ayatniryat_sum(request):
    ...
    if request.user.role == 'DE':
        sum_values = AayatNiryat.objects.filter(
            ...
            total = Sum('m_pragati') + Sum('pratikul_n') + Sum('pratikul_p') 
        )
...
def rbpa_monthly_sum(request, month):
    ...
    if request.user.role == 'DE':
        sum_values = DetailRbpa.objects.filter(
            ...
            total_quantity =  Sum('quantity1') + Sum('quantity2') + Sum('quantity3'),
            total_samples=Sum('sample1') + Sum('sample2') + Sum('sample3'),
            total_35_samples= Sum('sample1'),
            total_35_45_samples= Sum('sample2'),
            total_45_samples= Sum('sample3'),
            total_35_quantity= Sum('quantity1'),
            total_35_45_quantity= Sum('quantity2'),
            total_45_quantity=Sum('quantity3'),
        )
    elif request.user.role == 'A':
        sum_values = DetailRbpa.objects.filter(
            is_verified = True, created_on_np_date__startswith=f'2080',
            related_month=month,
        ).aggregate(
            total_quantity =  Sum('quantity1') + Sum('quantity2') + Sum('quantity3'),
            total_samples=Sum('sample1') + Sum('sample2') + Sum('sample3'),
...
def rbpa_quarterely_report(request,quarter):
 
    months=quarter_dict[quarter]
    if request.user.role == 'DE':
        # Initialize total sums for each category
        total_sum = {
            'total_quantity': 0,
            'total_samples': 0,
            'total_35_samples': 0,
            'total_35_45_samples': 0,
            'total_45_samples': 0,
            'total_35_quantity': 0,
            'total_35_45_quantity': 0,
            'total_45_quantity': 0,
        }
        ...
        for month in months:
            ...
            try:
                ...
                # Accumulate sums for each category
